...
describe("CodexController", function () {
  beforeEach(async function (ctx) {
    ...
    ctx.CodexRunManager = {
      ...
      getSecurityOptions: sinon.stub().returns({
        approvalPolicy: "on-request",
        sandboxMode: "workspace-write",
        networkAccess: true,
      }),
      ...
      queueFollowUp: sinon.stub().resolves({ id: "run-1" }),
    ...
    vi.doMock("../../../../app/src/Features/Codex/CodexRunManager.mjs", () => ({
  ...
  it("starts a project Codex run", async function (ctx) {
    ...
    ctx.req.body.continueRunId = "run-parent";
  ...
  it("queues a follow-up for a running Codex run", async function (ctx) {
    ctx.req.params.Project_id = ctx.projectId;
    ctx.req.params.runId = "run-1";
    ctx.req.body.prompt = "Continue after the current tool";
    ctx.req.body.mode = "after_next_tool";
    ctx.req.body.options = {
      effort: "medium",
      summary: "auto",
      autoApply: true,
    ...
    await ctx.CodexController.queueFollowUp(ctx.req, ctx.res, ctx.next);

    expect(ctx.CodexRunManager.queueFollowUp).to.have.been.calledWith(
      sinon.match({
        userId: ctx.userId,
        projectId: ctx.projectId,
        runId: "run-1",
        prompt: "Continue after the current tool",
        mode: "after_next_tool",
        options: {
          effort: "medium",
          summary: "auto",
          autoApply: true,
